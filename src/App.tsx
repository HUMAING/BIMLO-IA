...
const ROLES = {
  cad: {
    name: "WALA", title: "Ingénieur Mécanique / CAD", emoji: "⚙️",
    docs: ["PDF Plans Fab", "IFC v2x3/4", "DWG/DXF", "Rapport paysager"],
    checks: [
      { id: 1, text: "Plans PDF exportés avec cartouche et révision", type: "human" },
      { id: 2, text: "Fichier IFC v2x3 ou IFC4 conforme BIM BIMLO", type: "human" },
      { id: 3, text: "Dimensions pylône conformes aux specs client", type: "human" },
      { id: 4, text: "Nomenclature matériaux acier S355/S235 complète", type: "human" },
      { id: 5, text: "Tolérances de fabrication annotées (ISO 2768)", type: "human" },
      { id: 6, text: "Intégration paysagère — gabarit visuel validé", type: "human" },
    ],
...
Réponds UNIQUEMENT avec le JSON array, rien d'autre.`
  ...
  struct: {
    name: "SEIF", title: "Ingénieur Structure", emoji: "🏗️",
    docs: ["Note de calcul PDF", "EN1993 rapport", "Cross-check FAB"],
    checks: [
      { id: 1, text: "Note de calcul charges permanentes G conforme EN1991", type: "human" },
      { id: 2, text: "Charges climatiques vent/neige zone calculées", type: "human" },
      { id: 3, text: "Vérification ELU/ELS acier EN1993-3-1 pylônes", type: "human" },
      { id: 4, text: "Fondations béton — descente de charge OK", type: "human" },
      { id: 5, text: "Connexion calcul ↔ plans fabrication vérifiée", type: "cross" },
    ],
...
Réponds UNIQUEMENT avec le JSON array, rien d'autre.`
  ...
  bimlo: {
    name: "BIMLO (CEO)", title: "Directeur Technique — Visa Final", emoji: "👁️",
    docs: ["Dossier complet FAB", "Note calcul visée", "Rapport IA synthèse", "Visa final BIMLO"],
    checks: [
      { id: 1, text: "Cohérence globale plans + calculs + IFC vérifiée", type: "final" },
      { id: 2, text: "Conformité réglementaire pylônes télécoms (ICPE/Urbanisme)", type: "final" },
      { id: 3, text: "Intégration paysagère approuvée — rapport photo joint", type: "final" },
      { id: 4, text: "Rapport IA synthèse lu et validé", type: "final" },
      { id: 5, text: "Signature électronique & archivage BIMLO confirmé", type: "final" },
    ],
...
const makeInitialState = () => ({
  cad:    { checks: ROLES.cad.checks.map(c => ({ ...c, done: false })),    aiResults: [], aiScore: 0, aiRun: false, validated: false, sentToStruct: false },
  struct: { checks: ROLES.struct.checks.map(c => ({ ...c, done: false })), aiResults: [], aiScore: 0, aiRun: false, validated: false, sentToBimlo: false },
  bimlo:  { checks: ROLES.bimlo.checks.map(c => ({ ...c, done: false })),  aiResults: [], aiScore: 0, aiRun: false, validated: false },
});

export default function App() {
  const [state, setState] = useState(makeInitialState());
  const [scanning, setScanning] = useState({ cad: false, struct: false, bimlo: false });
  const [notification, setNotification] = useState(null);

  const showNotif = (msg) => { setNotification(msg); setTimeout(() => setNotification(null), 4500); };

  const resetAll = () => { setState(makeInitialState()); showNotif("↺ Nouveau projet initialisé"); };

  const toggleCheck = (role, id) => {
    if (state[role].validated) return;
    setState(s => ({ ...s, [role]: { ...s[role], checks: s[role].checks.map(c => c.id === id ? { ...c, done: !c.done } : c) } }));
  };

  const runAI = async (role) => {
    setScanning(s => ({ ...s, [role]: true }));
    setState(s => ({ ...s, [role]: { ...s[role], aiResults: [], aiScore: 0, aiRun: false } }));

    const r = state[role];
    const checked   = r.checks.filter(c => c.done).map(c => c.text).join(", ") || "Aucun";
    const unchecked = r.checks.filter(c => !c.done).map(c => c.text).join(", ") || "Aucun";
    const prompt = ROLES[role].aiPrompt.replace("{CHECKED}", checked).replace("{UNCHECKED}", unchecked);
    ...
    try {
      ...
      const score = Math.min(99, results.reduce((acc, r) => acc + (r.type === "ok" ? 20 : r.type === "warn" ? 8 : r.type === "info" ? 5 : 0), 0));
      setScanning(s => ({ ...s, [role]: false }));
      setState(s => ({ ...s, [role]: { ...s[role], aiResults: results, aiScore: score, aiRun: true } }));
      showNotif(`🤖 IA ${ROLES[role].name} — Analyse réelle terminée · Score ${score}%`);
    } catch (err) {
      setScanning(s => ({ ...s, [role]: false }));
      setState(s => ({ ...s, [role]: { ...s[role], aiResults: [{ type: "err", msg: "Erreur API — vérifier la connexion" }], aiRun: true, aiScore: 0 } }));
  ...
  const validate = (role) => {
    ...
    if (role === "cad")    { setTimeout(() => setState(s => ({ ...s, cad: { ...s.cad, sentToStruct: true } })), 400); showNotif("✅ Plans FAB transmis à SEIF — Ingénieur Structure"); }
    if (role === "struct") { setTimeout(() => setState(s => ({ ...s, struct: { ...s.struct, sentToBimlo: true } })), 400); showNotif("✅ Note de calcul transmise à BIMLO — Visa final"); }
    if (role === "bimlo")  showNotif("🏆 DOSSIER VALIDÉ BIMLO — Archivage lancé !");
  };

  const totalDone  = Object.values(state).reduce((a, r) => a + r.checks.filter(c => c.done).length, 0);
  const totalItems = Object.values(ROLES).reduce((a, r) => a + r.checks.length, 0);
  const aiDone     = [state.cad.aiRun, state.struct.aiRun, state.bimlo.aiRun].filter(Boolean).length;
  const valDone    = [state.cad.validated, state.struct.validated, state.bimlo.validated].filter(Boolean).length;
  ...
  const RoleCard = ({ role, cardClass }) => {
    ...
    const isLocked = (role === "struct" && !state.cad.sentToStruct) || (role === "bimlo" && !state.struct.sentToBimlo);

    return (
      <div className={`card ${cardClass}`} style={isLocked ? { opacity: 0.4, pointerEvents: "none" } : {}}>
        <div className={`card-header ${role}`}>
          ...
          {isLocked
            ? <span style={{ fontSize: 10, color: "#374151", fontFamily: "monospace" }}>🔒 EN ATTENTE</span>
            : <div className={`status-dot ${r.validated ? "done" : allDone && r.aiRun ? "active" : "pending"}`} />
          }
        </div>

        <div className="card-body">
          <div className="section-title">📁 Documents livrables</div>
          <div className="tag-row">{R.docs.map(d => <span key={d} className="tag">{d}</span>)}</div>
          ...
          <div className="checklist">
            {r.checks.map(c => (
              <div key={c.id} className={`check-item ${c.done ? "checked" : ""}`} onClick={() => toggleCheck(role, c.id)}>
                <div className="check-icon">{c.done ? "✓" : ""}</div>
                <span className="check-text">{c.text}</span>
                <span className={`check-badge badge-${c.type}`}>
                  {c.type === "human" ? "PERSO" : c.type === "cross" ? "CROISÉ" : "FINAL"}
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="ai-block">
            <div className="ai-header">
              ...
              {r.aiRun && <span style={{ marginLeft: "auto", fontSize: 12, color, fontFamily: "monospace", fontWeight: 600 }}>{r.aiScore}%</span>}
              {isScanning && <span style={{ marginLeft: "auto", fontSize: 10, color: "#6b7a94", fontFamily: "monospace" }}>analyse<span className="typing">▋</span></span>}
            </div>

            {(isScanning || r.aiRun) && (
              <div className="ai-scan-bar">
                <div className={`ai-scan-fill fill-${role}`} style={{ width: isScanning ? "65%" : "100%" }} />
              </div>
            )}

            <div className="ai-findings">
              {!isScanning && !r.aiRun && <div className="finding info">○ &nbsp;Cliquez "Lancer IA" pour analyse Claude réelle…</div>}
              {isScanning && <div className="finding info">⟳ &nbsp;Claude analyse vos données en temps réel…</div>}
              {!isScanning && r.aiRun && r.aiResults.map((f, i) => (
                <div key={i} className={`finding ${f.type}`}>
                  {f.type === "ok" ? "✓" : f.type === "warn" ? "⚠" : f.type === "err" ? "✗" : "○"}&nbsp;&nbsp;{f.msg}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="action-bar">
            <button
              ...
              onClick={() => !isScanning && !r.aiRun && !r.validated && runAI(role)}
              disabled={isScanning || r.aiRun || r.validated}
            >
              {isScanning ? "⟳ Claude analyse…" : r.aiRun ? "✓ IA Complète" : "▶ Lancer IA Claude"}
            </button>
            <button
            ...
            >
              {r.validated ? "✓ Validé" : role === "bimlo" ? "🏆 Visa final BIMLO" : "Transmettre →"}
            </button>
          </div>

          {r.validated && (
            <div className="validated-banner">
              ✓ {role === "cad" ? "Plans FAB transmis à SEIF" : role === "struct" ? "Note transmise à BIMLO" : "🏆 DOSSIER VALIDÉ — Archivé BIMLO"}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="app">
        <div className="header">
          ...
          <div className="header-title">
            ...
            <button className="reset-btn" onClick={resetAll}>↺ Nouveau projet</button>
          </div>
          <div className="header-sub">Plans Fabrication PDF · Note de Calcul · IFC · Intégration Paysagère · <span style={{ color: "#00c8ff" }}>IA Claude Réelle ✓</span></div>
        </div>

        {/* Pipeline */}
        <div className="pipeline">
          {[
            { badge: "W", role: "cad",    active: !state.cad.validated,                          label: "WALA",  sub: "Ing. CAD" },
          ].map(p => (
            <div key={p.role} className="pipeline-step">
              <div className="pipeline-node">
                <div className={`pipeline-badge ${p.role} ${p.active ? "active" : ""}`}>{p.badge}</div>
                <div className="pipeline-label"><strong>{p.label}</strong>{p.sub}</div>
              </div>
            </div>
          ))}
          <div className="pipeline-arrow" />
          <div className="pipeline-step"><div className="pipeline-node">
            <div style={{ width:34,height:34,borderRadius:6,background:"#0d1929",border:"1.5px dashed #1e2d3d",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15 }}>🤖</div>
            <div className="pipeline-label" style={{fontSize:9}}>IA<br/>FAB</div>
          </div></div>
          <div className="pipeline-arrow" style={{ width:28 }} />

          <div className="pipeline-step"><div className="pipeline-node">
            <div className={`pipeline-badge struct ${state.cad.sentToStruct && !state.struct.validated ? "active" : ""}`}>S</div>
            <div className="pipeline-label"><strong>SEIF</strong>Ing. Structure</div>
          </div></div>
          <div className="pipeline-arrow" />
          <div className="pipeline-step"><div className="pipeline-node">
            <div style={{ width:34,height:34,borderRadius:6,background:"#0d1929",border:"1.5px dashed #1e2d3d",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15 }}>🤖</div>
            <div className="pipeline-label" style={{fontSize:9}}>IA<br/>Calcul</div>
          </div></div>
          <div className="pipeline-arrow" style={{ width:28 }} />

          <div className="pipeline-step"><div className="pipeline-node">
            <div className={`pipeline-badge ceo ${state.struct.sentToBimlo && !state.bimlo.validated ? "active" : ""}`}>B</div>
            <div className="pipeline-label"><strong>BIMLO</strong>CEO Visa</div>
          </div></div>
          <div className="pipeline-arrow" />
          <div className="pipeline-step"><div className="pipeline-node">
            <div style={{ width:34,height:34,borderRadius:6,background:"#0d1929",border:"1.5px dashed #1e2d3d",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15 }}>🤖</div>
            <div className="pipeline-label" style={{fontSize:9}}>IA<br/>Synthèse</div>
          </div></div>
          <div className="pipeline-arrow" style={{ width:28 }} />

          <div className="pipeline-step"><div className="pipeline-node">
            <div style={{ width:52,height:52,borderRadius:6,background:state.bimlo.validated?"rgba(16,185,129,0.2)":"#111825",border:`2px solid ${state.bimlo.validated?"#10b981":"#1a2535"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,transition:"all 0.4s" }}>
              {state.bimlo.validated ? "✅" : "📁"}
            </div>
            <div className="pipeline-label"><strong style={{color:state.bimlo.validated?"#10b981":undefined}}>{state.bimlo.validated?"ARCHIVÉ":"ARCHIVE"}</strong>BIMLO</div>
          </div></div>
        </div>
        ...
        <div className="progress-summary">
          <div className="prog-card">
            <div className="prog-num" style={{color:"#00c8ff"}}>{totalDone}/{totalItems}</div>
            <div className="prog-label"><strong>Points vérifiés</strong>toutes disciplines</div>
          </div>
          <div className="prog-card">
            <div className="prog-num" style={{color:"#10b981"}}>{aiDone}/3</div>
            <div className="prog-label"><strong>Analyses IA Claude</strong>complétées</div>
          </div>
          <div className="prog-card">
            <div className="prog-num" style={{color:"#a78bfa"}}>{valDone}/3</div>
            <div className="prog-label"><strong>Validations</strong>signées</div>
          </div>
        </div>
